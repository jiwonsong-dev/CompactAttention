...
def _sync_qwen_base_vocab_size(config: SeerAttnQwen2Config, base_model: str, kwargs) -> None:
    config_kwargs = {}
...
def _resolve_qwen_seer_device_map(config, device_map):
    visible_gpu_count = torch.cuda.device_count()
    if device_map != "auto" or visible_gpu_count != 2:
        return device_map
    ...
    # Keep shared rotary embeddings with the first contiguous decoder block.
    split_start = max(1, (num_layers // 2) - 1)
    resolved_device_map = {
        "model.embed_tokens": 0,
        "model.rotary_emb": 0,
        "model.block_rotary_emb": 0,
    }
    for layer_idx in range(num_layers):
        resolved_device_map[f"model.layers.{layer_idx}"] = 0 if layer_idx < split_start else 1
    resolved_device_map["model.norm"] = 1
    resolved_device_map["lm_head"] = 1
            ...
            if self.config.use_flash_rope:
                ...
                if cos.shape[0] == 1:
                    ...
                    if torch.equal(position_ids, ref_position_ids.expand_as(position_ids)):
                        ...
                        raise ValueError(
                            "flash_rope with batch_size>1 currently requires identical position_ids across batch"
                        )
            else:
...
class SeerAttnQwen2ForCausalLM(SeerAttnQwen2PreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, load_gate=True, *model_args, **kwargs):
        ...
        if load_gate:
            ...
            kwargs["device_map"] = _resolve_qwen_seer_device_map(kwargs.get("config", None), kwargs.get("device_map"))
