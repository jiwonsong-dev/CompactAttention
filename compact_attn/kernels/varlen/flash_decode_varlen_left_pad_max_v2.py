...
def flash_decode_leftpad(
    ...
    force_triton = os.environ.get("SEERATTN_DEBUG_FORCE_TRITON_LEFTPAD_DECODE", "0") == "1"
    ...
    cache_seqlens = cache_seqlens.to(device=q.device, dtype=torch.int32).contiguous().view(batch)

    # Prefer the FA2 decode path for regular KV-cache decode. The custom Triton kernel
    # assumes a left-padded layout and has shown intermittent illegal accesses on the
    # generation path; FA2 matches the cache_seqlens semantics we use elsewhere.
    if not force_triton:
